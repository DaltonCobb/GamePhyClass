...
{
    public static void Reslove(List<Contact> contacts)
    {
        ...
        { 
            ...
            //collision impulse
            Vector2 relativeVeocity = contact.bodyA.velocity - contact.bodyB.velocity;
            float normalVeocity = Vector2.Dot(relativeVeocity, contact.normal);

            if (normalVeocity > 0) continue;

            float restitution = (contact.bodyA.restitution + contact.bodyB.restitution) * 0.5f;

            float impulseMagnitude = -(1.0f + restitution) * normalVeocity / totalInverseMass;
